...
export default function SearchDialog({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-xl bg-bg-surface border border-border rounded-card shadow-2xl overflow-hidden"
      ...
      >
        <div className="flex items-center border-b border-border px-4">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-0 outline-none px-3 py-3 text-white placeholder-zinc-600"
          />
        ...
        <div className="max-h-[50vh] overflow-y-auto p-2">
          {Object.entries(grouped).length === 0 && (
            <p className="text-center text-zinc-500 py-8 text-sm">No results found.</p>
          )}
          {Object.entries(grouped).map(([section, items]) => (
            <div key={section} className="mb-2">
              <p className="text-xs font-semibold text-zinc-600 uppercase tracking-wider px-2 py-1.5">
                {section}
              </p>
              {items.map((item) => {
